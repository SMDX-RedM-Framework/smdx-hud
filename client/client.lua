...
CreateThread(function()
    while true do
        ...
        if LocalPlayer.state.isLoggedIn and incinematic == false and inBathing == false and inClothing == false and showUI then
            ...
            -- outlaw status
            local outlawstatus = SMDXCore.Functions.GetPlayerData().metadata['outlawstatus']
...
CreateThread(function()
    while true do
        ...
        local IsBirdPostApproaching = exports['smdx-telegram']:IsBirdPostApproaching()
...
RegisterNetEvent('hud:client:OnMoneyChange', function(type, amount, isMinus)
    ...
    SendNUIMessage({
        action = 'update',
        cash = SMDXCore.Shared.Round(cashAmount, 2),
        bloodmoney = SMDXCore.Shared.Round(bloodmoneyAmount, 2),
        bank = SMDXCore.Shared.Round(bankAmount, 2),
        amount = SMDXCore.Shared.Round(amount, 2),
        minus = isMinus,
        type = type,
    })
end)
...
CreateThread(function()
    while true do
        if LocalPlayer.state.isLoggedIn then
            SMDXCore.Functions.TriggerCallback('hud:server:getTelegramsAmount', function(amount)
                if amount > 0 then
                    youhavemail = true
                else
                    youhavemail = false
...
CreateThread(function()
    while true do
        if LocalPlayer.state.isLoggedIn then
            ...
            local isBathingActive = exports['smdx-bathing']:IsBathingActive()
            local IsCothingActive = exports['smdx-appearance']:IsCothingActive()
